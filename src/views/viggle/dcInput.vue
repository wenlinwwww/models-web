...
import { NButton, NDrawer, NDrawerContent, NImage, NInput, NSelect, useMessage } from 'naive-ui'
import dcTemple from './dcTemple.vue'
import { useBasicLayout } from '@/hooks/useBasicLayout'
import { SvgIcon } from '@/components/common'
import { gptServerStore, homeStore } from '@/store'
import { mlog, upImg } from '@/api'
import type { ViggleTemplate } from '@/api/viggle'
import { FeedViggleTask, viggleFetch } from '@/api/viggle'
...
const generate = async () => {
  ...
  const d = await viggleFetch(getMyProUrl('/video-task'), f.value)
...
async function selectFileVideo(input: any) {
  ...
  const d: any = await viggleFetch(getMyProUrl(`/asset/video/${f.value.imageID}`), formData, { upFile: true })
  mlog('d Video', d)
  if (d.data) {
    if (d.data.id) {
...
async function selectFile(input: any) {
  try {
    const ud = await upImg(input.target.files[0])
    const formData = new FormData()
    formData.append('file', input.target.files[0])

    const d: any = await viggleFetch(getMyProUrl('/asset/image'), formData, { upFile: true })
    mlog('d Image', d)
    if (d.data) {
      if (d.data.id) {
...
const isHK = computed(() => {
  ...
  if (url != '')
    return (url.includes('hk') && !url.includes('pro'))
...
<template>
  <div class="p-2">
    <div>
      <NSelect v-model:value="f.modelInfoID" :options="modelOption" size="small" />
    </div>
    <div class="pt-2">
      <NSelect v-model:value="f.bgMode" :options="bgOption" size="small" />
    </div>
    <div class="pt-2 flex justify-center items-center w-full relative">
      ...
      <div v-if="st.imgSrc" class="absolute right-1 top-3 z-40">
        <NButton strong secondary round size="small" type="success" @click="clear(2)">
          {{ $t('common.clear') }}
        </NButton>
      </div>
    ...
    <div v-if="useTem" class="pt-2 ">
      <div class="relative h-[180px]">
        <div class="absolute right-1 top-1 z-40">
          <NButton strong secondary round size="small" type="success" @click="clear(1)">
            {{ $t('common.clear') }}
          </NButton>
        </div>
        <div class=" absolute w-full h-full top-0 left-0 z-1">
          <video
            ...
            class="  rounded-lg bg-[#242424] object-contain w-full h-full transition-all" :src="useTem.processedURL"
          />
        </div>
      </div>
    </div>
    <div v-else-if="st.vgSrc" class="pt-2 ">
      <div class="relative h-[180px]">
        <div class="absolute right-1 top-1 z-40">
          <NButton strong secondary round size="small" type="success" @click="clear(3)">
            {{ $t('common.clear') }}
          </NButton>
        </div>
        <div class=" absolute w-full h-full top-0 left-0 z-1">
          <video
            ...
            class="  rounded-lg bg-[#242424] object-contain w-full h-full transition-all" :src="st.vgSrc"
          />
        </div>
      </div>
    </div>
    ...
    <div v-else class="pt-2 flex justify-center items-center w-full">
      ...
      <div class="h-[180px] w-full overflow-hidden rounded-sm border border-gray-400/20 flex justify-center items-center ">
        ...
        <div class="text-center">
          <div class="cursor-pointer p-2" @click="selectVideo">
            {{ $t('dance.upvideo') }}
          </div>
          <NButton type="primary" size="small" @click="st.showImg = true">
            {{ $t('dance.usevideo') }}
          </NButton>
        </div>
      </div>
    ...
    <div v-if="isHK" class="pt-2">
      <NSelect v-model:value="st.version" :options="mvOption" size="small" />
    </div>
      ...
      <template #header>
        <div class="flex justify-between items-center w-full">
          ...
          <div class=" max-w-[400px]">
            <NInput v-model:value="st.q" round :placeholder="t('dance.moban2')" clearable @keydown.enter="search()">
              <template #prefix>

  ...
  <div class="p-2">
    ...
    <div v-if="exLuma" class="pt-1">
      ...
      <div class="relative flex items-center justify-center bg-white bg-opacity-10 rounded-[5px] overflow-hidden aspect-[16/8.85] ">
        <video v-if="exLuma.video?.url || exLuma.video?.download_url" :src="exLuma.video?.download_url ? exLuma.video?.download_url : exLuma.video?.url" loop playsinline controls class="w-full h-full object-cover" @error="$event.target.src = exLuma.video?.url" />
      </div>
    </div>
    ...
    <div class="pt-1">
      <div class="flex justify-between items-end">
        <div class="pb-1 text-right">
          <NTag v-if=" exLuma || luma.user_prompt != '' || luma.image_url != '' || luma.image_end_url != ''" type="success" size="small" round>
